...
def draw_horizontal_choke(ax, x_center, y_center, label='CHOKE', box_width=0.45, box_height=0.35):
    ...
    # Optional connection lines (shortened for better fit; adjust delta/line_length if needed)
    line_length = 0.075  # Short to avoid overlapping nearby elements
    delta = 0.02
    ax.plot([left_x - line_length - delta, left_x - delta], [y_center, y_center], color='black', linewidth=1.4)
    ax.plot([right_x + delta, right_x + line_length + delta], [y_center, y_center], color='black', linewidth=1.4)
    ...
    for letter, circuit_list in letter_groups.items():
        ...
        for circuit_id in circuit_list:
            ...
            if not choke_row.empty and str(choke_row['choke'].iloc[0]).strip().lower() == 'yes':
                ...
                if input_term in terminal_names_for_positions and output_term in terminal_names_for_positions:
                    ...
                    box_width = max(1.2, x_right - x_left - 0.2)  # Ensure minimum width; adjust as needed
                    choke_label = str(choke_row['terminal_name'].iloc[0]).strip() if not choke_row.empty else 'CHOKE'
                    draw_horizontal_choke(ax, x_center, y_bottom_bus_group, label=choke_label, box_width=box_width)
