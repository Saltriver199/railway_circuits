...
def draw_relay_output(ax, x_left, x_right, y=0, scale=1.0, text='RELAY', anchor_to_v_tip=False, v_offset=0.4):
    ...
    if len(display_text) >= 8:
        ...
        if len(words) > 1:
            # Join first word(s) in line1, rest in line2
            line1 = " ".join(words[:-1])
            line2 = words[-1]
            display_text = f"{line1}\n{line2}"
        else:
            # If no spaces, just break after 8 chars
...
# === Load Excel ===
EXCEL_FILE = 'C:\Diagram\RAILWAYPROJECT.xlsx'
...
def draw_header(ax, circuit_id, header_type, x_start, x_end, text, min_symbol_bottom=None,
    ...
    # Normalize text: if NaN/empty -> do nothing
...
def draw_capsule(ax, x, y_center, terminal_name, input_left, input_right, output_left, output_right,
    ...
    # Draw bottom circle with slightly smaller radius
    ...
    # Draw input and output labels with multi-line support
...
# === Modified dual_fuse: small inner gap between fuses ===
# === Modified dual_fuse: small inner gap between fuses ===
def draw_dual_fuse(ax, x_left, y_center, left_term, right_term):
    """
    Draws two fuses very close to each other (small inner gap).
    x_left is the center x of the LEFT fuse. RIGHT fuse center is x_left + inner_spacing.
    Function still returns left-side connection points so vertical hooks attach to left fuse.
    """
    left_row = find_row_by_term(left_term)
    right_row = find_row_by_term(right_term)

    # Get individual connection flags
    ...
    # INNER spacing between the two fuse centers (make this small to pull them together)
    INNER_SPACING_MULT = 2.8   # multiplier of SYMBOL_WIDTH for inner gap (tweak smaller/larger)
    ...
    def _draw_one_s(ax, x_pos, y_c, row, term_shift=0.0):
        ...
        # Circles slightly smaller than capsule
        top_circle_radius = SYMBOL_RADIUS * 0.8
        bottom_circle_radius = SYMBOL_RADIUS * 0.8
        ...
        # S-curve (Bezier) between top and bottom circles
        ...
        # connection points returned relative to the circle edges
    ...
    # Return left connection points for chaining (vertical hooks will use left fuse)
    top_conn = (x_left, top_rail_y)
    bottom_conn = (x_left, bottom_rail_y)

    # Use individual connection flags instead of combined ones
...
def draw_input_connection(ax, x, symbol_top_y, connected_flag, y_top_bus_group):
    ...
    Always connect to the top bus, no matter the flag.
    """
...
def draw_output_connection(ax, x, symbol_bottom_y, connected_flag, y_bottom_bus_group):
    ...
    Always connect to the bottom bus, no matter the flag.
    """
...
def draw_bus_lines(ax, x_positions, connected_flags, bus_y, gap=0.12, extra=0.12):
    """
    Draw contiguous bus segments across x_positions where connected_flags is True.
    Introduces a small visual gap at segment ends to indicate breaks (gap param).
    'extra' extends the line a little beyond the current segment.
    """
    if not x_positions:
        return
    segments = []
    ...
    for start_idx, end_idx in segments:
        ...
        # If single symbol, draw a short centered tick
        if start_idx == end_idx:
            small = max(0.06, gap)
            ax.plot([x_start - small, x_start + small], [bus_y, bus_y], color='black', linewidth=1)
        else:
            # shorten segment ends by gap to create visible break, but extend a bit with 'extra'
            total_len = x_end - x_start
            shrink = min(gap, total_len / 4.0)
            plot_start = x_start + shrink - extra   # extend a bit left
            plot_end = x_end - shrink + extra       # extend a bit right
            if plot_end <= plot_start:
                # fallback to tiny centered line
...
def draw_symbols(df_symbols, ax, ordered_circuit_ids, start_x=1, pin_spacing=0.8, circuits_per_page=12, page_number=1):
    ...
    for circuit_id in page_circuit_ids:
        ...
        while i < len(group):
            ...
            #    
                ...
                if i + 1 < len(group):
                    ...
                    # Get individual connection flags for both fuses
                    ...
                    # Store both terminal names separately
                    ...
                    # Add both positions (same x for both terminals in dual fuse)
                    ...
                    # Use individual connection flags for each terminal
                    ...
                    # fallback to single fuse if no next row
                    top_conn, bottom_conn, input_conn, output_conn = draw_s_fuse(
                        ax, current_x, capsule_y_center, row.get('terminal_name')
                    )
                    x_positions.append(current_x)
        ...
        hook_input_flags = []
        hook_output_flags = []
        # In the connection drawing section, add special handling for dual fuses:
        for j, x in enumerate(x_positions):
            # For dual fuses, only draw connections once per x position
        ...
        # Collect ranges for bus segments
        top_ranges = []
        bottom_ranges = []
        ...
        # Decide whether to merge adjacent ranges into one continuous rail.
        # If both WIREFROM and WIRETO headers exist for the circuit, keep adjacent ranges separate
        # so the bus line visually breaks between groups (this matches desired behavior).
        merge_adjacent = True
        ...
        # If no segments, use full if any connected
        ...
        # Replace the name_to_x creation with a more robust version that handles duplicates
        name_to_x = {}
        ...
        for idx, (xval, tname) in enumerate(zip(x_positions, terminal_names_for_positions)):
            # For dual fuses, we might have duplicate x positions
            if tname in name_to_x:
                # If we already have this terminal name, keep the first occurrence
                continue
        ...
        if not circuit_groups.empty:
            ...
            def parse_terminal_no_field(val):
                ...
                # Handle comma-separated values (like "61,65")
                if ',' in s:
                    parts = s.split(',')
                    if len(parts) >= 2:
                        try:
                ...
                # Handle hyphen-separated ranges (like "61-65")
                if '-' in s:
                    parts = s.split('-')
                    if len(parts) >= 2:
                        try:
        ...
        for _, hrow in circuit_headers.iterrows():
            ...
            x_left = min(x_positions[start_idx:end_idx+1])
            x_right = max(x_positions[start_idx:end_idx+1])
