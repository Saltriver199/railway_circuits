...
def draw_resistor(ax, x, y_center, input_terminal='', output_terminal='', resistor_name='R', input_x_pos=None, output_x_pos=None):
    ...
    # Upper horizontal line (dynamic to input_x_pos if provided) with vertical drop at start
    upper_label = str(input_terminal).strip()
    if input_x_pos is not None:
        left_x = min(input_x_pos, x)
        right_x = max(input_x_pos, x)
        ...
        upper_horiz_length = 0.5 + len(upper_label) * 0.12
    ...
    # Lower horizontal line (dynamic to output_x_pos if provided) with vertical rise at start
    lower_label = str(output_terminal).strip()
    if output_x_pos is not None:
        left_x = min(output_x_pos, x)
        right_x = max(output_x_pos, x)
        ...
        lower_horiz_length = 0.5 + len(lower_label) * 0.12
    ...
    for letter, circuit_list in letter_groups.items():
        ...
        for circuit_id in circuit_list:
            ...
            if not resistor_row.empty and str(resistor_row['resistor'].iloc[0]).strip().lower() == 'yes':
                ...
                input_terms = str(resistor_row['input_terminal'].iloc[0]).strip().replace('.0', '') if pd.notna(resistor_row['input_terminal'].iloc[0]) else ''
                output_terms = str(resistor_row['output_terminal'].iloc[0]).strip().replace('.0', '') if pd.notna(resistor_row['output_terminal'].iloc[0]) else ''
                input_x_pos = None
                if input_terms in terminal_names_for_positions:
                    input_x_pos = x_positions[terminal_names_for_positions.index(input_terms)]
                output_x_pos = None
                if output_terms in terminal_names_for_positions:
                    output_x_pos = x_positions[terminal_names_for_positions.index(output_terms)]
                symbols_to_add = 1
                if current_terminal_count + symbols_to_add > max_terminal_symbols_per_row:
                    row_index += 1
                    overall_max_x = max(overall_max_x, current_row_max_x)
                    ...
                    if row_index >= max_rows_visible:
                        ...
                        break
                current_x -= 0.5
                draw_resistor(ax, current_x, capsule_y_center, input_terminal=input_terms, output_terminal=output_terms, resistor_name=resistor_label, input_x_pos=input_x_pos, output_x_pos=output_x_pos)
                current_x += pin_spacing
                current_row_max_x = max(current_row_max_x, current_x)
                current_terminal_count += 1
            ...
            for _, hrow in circuit_headers.iterrows():
                ...
                if header_type == 'RELAY':
                    if input_output == 'input':
                        ...
                        # Check if this circuit has a choke
                        choke_info = df_choke[df_choke['circuit_id'] == circuit_id]
                        choke_output_terminal = None
                        if not choke_info.empty:
                            # Check if the current group's output terminal matches the choke's output terminal
                            ...
                            # Check if this terminal is in the current group range
                            start_name = str(hrow.get('terminal_start')).strip().replace('.0', '') if pd.notna(hrow.get('terminal_start')) else None
                            end_name = str(hrow.get('terminal_end')).strip().replace('.0', '') if pd.notna(hrow.get('terminal_end')) else None
                            
                            if start_name and end_name and output_terminal in [start_name, end_name]:
                                choke_output_terminal = output_terminal

                        draw_group_bottom_symbol(ax, x_left, x_right, vertical_line_end, text=text, 
