...
def generate_pdf_metadata_checksum(df_title, excel_file_path, pdf_file_path):
    ...
    try:
        ...
        # For now, use placeholder for content size (will be updated after PDF generation)
        content_size = 0
        
        # Get file name
        file_name = os.path.basename(pdf_file_path)
        
        # Create data string for checksum (the exact format you requested)
...
def update_pdf_checksum_metadata(pdf_file_path, checksum, checksum_data, content_size, df_title):
    ...
    try:
        ...
        actual_content_size = os.path.getsize(pdf_file_path)
        
        # Update checksum data with actual file size
        updated_checksum_data = f"{station_code}|{actual_content_size}|{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}|{os.path.basename(pdf_file_path)}"
        updated_checksum = hashlib.md5(updated_checksum_data.encode()).hexdigest()
        
        print(f"Final checksum with actual file size: {updated_checksum}")
        print(f"File size: {actual_content_size} bytes")
        
        return updated_checksum, updated_checksum_data, actual_content_size
...
def draw_rectangle_symbol(ax, x, y_center, terminal_name, input_left, input_right, output_left, output_right,
                          input_connected, output_connected):
...
# Generate initial checksum (with placeholder file size)
# Generate initial checksum (with placeholder file size)
output_file = 'Terminal_Symbols_Centered_Fixed_Size.pdf'
checksum, checksum_data, content_size = generate_pdf_metadata_checksum(df_title, EXCEL_FILE, output_file)
...
with PdfPages(output_file) as pdf:
    for page_num, (junction_name, page_circuit_ids) in enumerate(pages, 1):
        ...
        # Calculate page_start_x for centering (THIS WAS MISSING)
        shift = (global_max_width - (page_max_width + 1.2)) / 2
        page_start_x = 1 + shift  # NOW DEFINED
        ...
        # Draw footer - page_start_x is now defined
        left = page_start_x - 1.5  # NOW DEFINED
